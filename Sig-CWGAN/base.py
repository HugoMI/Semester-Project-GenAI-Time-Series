...
def get_standard_test_metrics(x):
    test_metrics_list = [
        ...
        test_metrics['cfvol_id'](x, max_lag=3, reg=0.5),
        test_metrics['vol_clust'](x, max_lag=3, reg=0.5),
        test_metrics['acf_id'](x, max_lag=3, reg=0.3),
        test_metrics['lev_eff'](x, max_lag=3, reg=0.3),
        test_metrics['mean'](x, reg=0.3),
        test_metrics['std'](x, reg=0.5),
        test_metrics['skew'](x, reg=0.5),
        test_metrics['kurtosis'](x, reg=0.6),
    ]
...
class BaseAlgo:
    def __init__(self, base_config, x_real):
        ...
        self.x_real = x_real
#         print("BaseAlgo self.x_real.shape", self.x_real.shape)
    ...
    def fit(self):
        ...
        for n_step in tqdm(range(1, self.total_steps + 1), ncols=80):  # sig_pred, x_past, x_real
            print("n_step", n_step)
            self.step(n_step)
...
#         print("BaseAlgo evaluate x_fake.shape", x_fake.shape)
    ...
    def plot_losses(self):
        fig, axes = plt.subplots(len(self.test_metrics_list) + 1, 1, figsize=(20, 16))
