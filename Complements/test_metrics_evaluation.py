...
def coarfine_vol_torch(x: torch.Tensor, max_lag: int, dim: Tuple[int] = (0, 1)) -> torch.Tensor:
    """
    ...
    def compute_coarse_volatility(x):
        coarse_volatility = []
        for i in range(x.shape[1]//delta):
            coarse_volatility.append(torch.abs(torch.sum(x[:, delta*i:delta*(i+1)], 1)))
        return coarse_volatility

    def compute_fine_volatility(x):
        fine_volatility = []
        for i in range(x.shape[1]//delta):
            fine_volatility.append(torch.sum(torch.abs(x)[:, delta*i:delta*(i+1)], 1)/delta)
        return fine_volatility

    def compute_correlation(x1,x2):
        x1_mean = torch.mean(x1, dim=(0, 1))
        x2_mean = torch.mean(x2, dim=(0, 1))
        x1_std = torch.std(x1, unbiased=False, dim=(0, 1))
        x2_std = torch.std(x2, unbiased=False, dim=(0, 1))
        return torch.mean((x1-x1_mean)*(x2-x2_mean), dim)/(x1_std*x2_std)
    

    lags = [i for i in range(min_lag,max_lag+1)]

    coarse_volatility = compute_coarse_volatility(x)
    coarse_volatility = torch.cat(coarse_volatility, dim=0)
    # print("coarse_volatility",coarse_volatility.shape,coarse_volatility)
    fine_volatility = compute_fine_volatility(x)
    fine_volatility = torch.cat(fine_volatility, dim=0)
    # print("fine_volatility",fine_volatility.shape,fine_volatility)

    values = []
    for lag in lags:
#         print("**lag**",lag)
        if lag == 0:
            value = compute_correlation(coarse_volatility,fine_volatility)
            values.append(torch.unsqueeze(value,0))
        elif lag > 0:
            value = compute_correlation(coarse_volatility[lag:],fine_volatility[:-lag])
            values.append(torch.unsqueeze(value,0))
        else:
            value = compute_correlation(coarse_volatility[:lag],fine_volatility[-lag:])
            values.append(torch.unsqueeze(value,0))
    # print(values)
    if dim == (0, 1):
#         print("cfv",torch.stack(values))
        return torch.stack(values)
    else:
        return torch.cat(values, 1)

def vol_clust_torch(x: torch.Tensor, max_lag: int, dim:Tuple[int]=(0, 1)) -> torch.Tensor:
    """
    ...
    for i in range(1, max_lag + 1):
        y = torch.square(x[:, i:]) * torch.square(x[:, :-i]) if i > 0 else torch.pow(x, 2)
...
#     print("max_lag",max_lag)
    ...
    for i in range(1, max_lag + 1):
        ...
        acf_list.append(acf_i)
    if dim == (0, 1):
#         print("acf",torch.stack(acf_list))
        return torch.stack(acf_list)
    else:
        return torch.cat(acf_list, 1)
...
def lev_eff_torch(x, max_lag, dim=(0, 1)):
    ...
    for i in range(1, max_lag + 1):
        ...
        lev_eff.append(l_i)
    if dim == (0, 1):
#         print("lev_eff", torch.stack(lev_eff))
...
class CFVolLoss(Loss):
    def __init__(self, x_real, max_lag=20, **kwargs):
        super(CFVolLoss, self).__init__(norm_foo=acf_diff, **kwargs)
        self.coarfine_vol_real = coarfine_vol_torch(self.transform(x_real), max_lag, dim=(0, 1))
        self.max_lag = max_lag

    def compute(self, x_fake):
        coarfine_vol_fake = coarfine_vol_torch(self.transform(x_fake), self.max_lag)
        return self.norm_foo(coarfine_vol_fake - self.coarfine_vol_real.to(x_fake.device))

    
class VCLoss(Loss):
    def __init__(self, x_real, max_lag=64, **kwargs):
        super(VCLoss, self).__init__(norm_foo=acf_diff, **kwargs)
        self.vol_clust_real = vol_clust_torch(self.transform(x_real), max_lag, dim=(0, 1))
        self.max_lag = max_lag

    def compute(self, x_fake):
        vol_clust_fake = vol_clust_torch(self.transform(x_fake), self.max_lag)
        return self.norm_foo(vol_clust_fake - self.vol_clust_real.to(x_fake.device))
...
test_metrics = {
    'cfvol_id': partial(CFVolLoss, name='cfvol_id'),
    'vol_clust': partial(VCLoss, name='vol_clust'),
